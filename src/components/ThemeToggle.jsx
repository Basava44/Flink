import { useTheme } from "../hooks/useTheme";

function ThemeToggle() {
  const { isDark, toggleTheme } = useTheme();

  const handleToggle = () => {
    console.log('ThemeToggle button clicked');
    toggleTheme();
  };

  return (
    <div className="fixed top-6 right-6 z-50">
      <button
        onClick={handleToggle}
        className={`group relative p-3 rounded-xl transition-all duration-200 shadow-soft hover:shadow-soft-lg ${
          isDark
            ? "bg-slate-800/50 hover:bg-slate-700/50 border border-slate-700/50"
            : "bg-white/80 hover:bg-white border border-gray-200"
        }`}
      >
        <div className="relative w-5 h-5">
          {isDark ? (
            <svg
              className="w-5 h-5 text-yellow-400 group-hover:scale-110 transition-transform duration-200"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                strokeWidth={2}
                d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z"
              />
            </svg>
          ) : (
            <svg
              className="w-5 h-5 text-slate-600 group-hover:scale-110 transition-transform duration-200"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                strokeWidth={2}
                d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z"
              />
            </svg>
          )}
        </div>
      </button>
    </div>
  );
}

export default ThemeToggle;